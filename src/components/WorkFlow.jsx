          ...
          {workflow.map((item) => (
            <div className="relative flex flex-col gap-2 items-start bg-primary/5 hover:bg-primary/10 rounded-lg p-5 hover:scale-105 transition-all duration-300">
              <div className="absolute -z-0 flex justify-center items-center h-[7rem] w-[7rem] md:h-[9rem] md:w-[9rem] rounded-full bg-primary/10 bottom-1 right-1">
                <span className="text-7xl md:text-8xl text-white/20 font-extrabold">
                  {item.id}
                </span>
              </div>
              <div className="z-10 w-[4rem] h-[4rem] p-3 rounded-full bg-white/70 flex items-center justify-center">
                <img
                  loading="lazy"
                  width="50"
                  height="50"
                  src={item.icon}
                  alt="icon"
                  className="w-[3rem] grayscale brightness-0"
                />
              </div>
              <h6 className="z-10 font-medium text-xl">{item.title}</h6>
              <p className="z-10 text-gray-300 text-sm">{item.description}</p>
            </div>
          ))}
        </div>
      </div>
